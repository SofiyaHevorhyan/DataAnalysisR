...
summary(mod3)
# confusion
c.m1 <- confusionMatrix(as.factor(round(mod1$fitted.values)),
                        as.factor(train1$Dependent))
c.m2 <- confusionMatrix(as.factor(round(mod2$fitted.values)),
                        as.factor(train2$Dependent))
c.m3 <- confusionMatrix(as.factor(round(mod3$fitted.values)),
                        as.factor(train3$Dependent))
...
hoslem.test(train_new$Dependent,mod1$fitted.values, g=10)
```
...
pred_prob<-predict (mod2, newdata=test_new, type="response")
...
pred_CAX<- predict(mod2, newdata=test, type="response")
