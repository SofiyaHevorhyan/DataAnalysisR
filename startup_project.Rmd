...
Now, we are trying to take closer look at our numerical data to understand which of the variables are the most important
```{r}
...
name_var <- rownames(importance1)[apply(importance1, 1, function(x) x > 2)]
name_var <- c(name_var, rownames(importance2)[apply(importance2, 1, function(x) x > 2.75)])
train1 <- cnt_df[, c(name_var, "dependent.company.status")]
```
...
train<-cnt_df[, c(final_var, "dependent.company.status")]
```
## Model Building
```{r}
# fitting stepwise binary logistic regression with logit link function
mod<-step(glm(dependent.company.status~., 
              family = binomial(link=logit),data = train))
# model summary
summary(mod)
# final logistic regression model
model<-glm(formula = dependent.company.status ~ team.size.senior.leadership +
             percent_skill_data.science + number.of.investors.in.seed +
             percent_skill_engineering + percent_skill_sales +
             percent_skill_leadership + percent_skill_domain +
             renown.score + number.of.co.founders + 
