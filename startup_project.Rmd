...
my.col.change.as.factor <- function(dataframe, col.name) {
  dataframe[,col.name] <- trimws(tolower(dataframe[,col.name])) # change all column names to lowercase
...
### Dealing with string
```{r}
char.col <- c(6:9, 16:17, 62) #save text columns separately
```
### Missing values
...
### PCA
```{r}
new <- cnt_df[-c(42, 43, 45)]
new[is.na(new)] <- 0.01
startup_pca <- prcomp(new, scale.=T, center=T)
startup_pca$sdev
startup_pca$rotation
plot(startup_pca, type="l")
summary(startup_pca)
# library(devtools)
...
We decided to perform PCA to see what are the principal components. However, the dataset is too wide to provide good and valuable results whhich describe high percent of population variance. After dozens of failed attempts, we decided to perform PCA on small part of our data, in particular, just column characteristics that describe different skills percentage for founders and co-founders of startups.
```{r}
#head(cnt_df)
...
## Random Forest for defining importance of variables
